...
int main(int argc, char **argv)
{
  ...
  // allocate memory for vocab and M
  fscanf(f, "%lld", &words);
  ...
  while (1) {
    ...
    // read a character string eval_data till a whiteshpace as the first string in the 4 strings tuple
    scanf("%s", st1);
    for (a = 0; a < strlen(st1); a++) st1[a] = toupper(st1[a]);
    // if end of a question series or file reached
    if ((!strcmp(st1, ":")) || (!strcmp(st1, "EXIT")) || feof(stdin)) {
      if (TCN == 0) TCN = 1;
      // output the result for the last question serires
      if (QID != 0) {
    ...
    // read the 2nd, 3rd, 4th string in the 4 strings tuple
    scanf("%s", st2);
