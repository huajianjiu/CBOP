//
// A trial to transfer a trained lexicon-based model to corpus-based model by EWC algorithm
// The copyright of the word2vec part is Google's.
// Created by ke on 17/05/19.
//
...
long long train_words = 0, word_count_actual = 0, l_iter = 10, c_iter = 5, file_size = 0, classes = 0;
...
int main(int argc, char **argv) {
  ...
  if (argc == 1) {
    ...
    printf("\t-l_iter <int>\n");
    printf("\t\tRun more training iterations for lexicon (default 5)\n");
    printf("\t-c_iter <int>\n");
    printf("\t\tRun more training iterations for corpus (default 5)\n");
    ...
    printf("\nExamples:\n");
    printf(" -train text9 -output vectors_ppdb2.bin -size 200 -window 8 -negative 25 -hs 0 -sample 1e-4 -l_iter 15 -c_iter 15\n\n");
    return 0;
  }
  output_file[0] = 0;
  save_vocab_file[0] = 0;
  ...
  if ((i = ArgPos((char *)"-l_iter", argc, argv)) > 0) l_iter = atoi(argv[i + 1]);
  if ((i = ArgPos((char *)"-c_iter", argc, argv)) > 0) c_iter = atoi(argv[i + 1]);
