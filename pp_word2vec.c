      ...
      if (cw) {
        ...
        if (negative_local > 0) for (d = 0; d < negative_local + 1 + PPDB_TABLE_SIZE; d++) {
          if (d == 0) {
            ...
            if (dropout == 1){
              // if over threshold fxik = 1

              if (paraphrase_scores[word * PPDB_TABLE_SIZE + d - 1] < para_threshold) fxik = 0;
              else fxik = 1;
              if (!fxik)
              {
                negative_local -= negative;
            ...
            if (paraphrases[word * PPDB_TABLE_SIZE] > 0){
              for (wi = 0; wi < PPDB_TABLE_SIZE; wi++){
                if (paraphrases[word * PPDB_TABLE_SIZE + wi] > 0 && target == paraphrases[word * PPDB_TABLE_SIZE + wi]) {
                  ...
                  if (dropout == 1){
                    if (paraphrase_scores[word * PPDB_TABLE_SIZE + d - 1] < para_threshold) sample_reject = 1;
