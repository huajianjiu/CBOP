...
void *TrainModelThread(void *id) {
    ...
    while (1) {
        ...
        for (d=0; d < PPDB_TABLE_SIZE; d++) {
            fxik = 0;
            if (dropout == 0){
                fxik = 1;
            } else if (dropout == 1) {
                if (paraphrase_scores[word * PPDB_TABLE_SIZE + d] < para_threshold) fxik = 0;
                else fxik = 1;
            }
            if ((fxik == 1) && (paraphrases[word * PPDB_TABLE_SIZE + d] > 0)) {
                for (c = 0; c < layer1_size; c++) neu1[c] += syn0[c + paraphrases[word * PPDB_TABLE_SIZE + d] * layer1_size];
                cw++;
            }
        }
        if (cw) {
            ...
            // lexicon part
            for (d=0; d < PPDB_TABLE_SIZE; d++) if (dropout > -1) {
                fxik = 0;
                if (dropout == 0){
                    fxik = 1;
                } else if (dropout == 1) {
                ...
                if ((fxik == 1) && (paraphrases[word * PPDB_TABLE_SIZE + d] > 0)) {
                    for (c = 0; c < layer1_size; c++) syn0[c + paraphrases[word * PPDB_TABLE_SIZE + d] * layer1_size] += neu1e[c];
                }
            }
        }
        sentence_position++;
